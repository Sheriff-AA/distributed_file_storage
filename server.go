...
func (s *FileServer) Get(key string) (io.Reader, error) {
	...
	for _, peer := range s.peers {
		// First read the file size so we can linit the amount of bytes that we read
...
func (s *FileServer) Store(key string, r io.Reader) error {
	...
	//2. broadcast file to all known peer in network
	var (
...
func (s *FileServer) handleMessageGetFile(from string, msg MessageGetFile) error {
	...
	fmt.Printf("[%s] serving file (%s) ove the network\n", s.Transport.Addr(), msg.Key)
	...
	// First send the "incomingStream" byte to the peer and teh we can send the file
	// size as an int64
	peer.Send([]byte{p2p.IncomingStream})
